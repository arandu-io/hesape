...
//
// It answers Illuminate\Console\BufferedConsoleOutput: the point is both halves
// at once. A command run from inside another one has to reach the terminal --
// otherwise the person watching sees a silent pause -- and the caller has to be
// able to read what it said, which is what Application::output does.
//
...
//
// It answers BufferedConsoleOutput::doWrite, which appends and then calls the
// parent. It is an io.Writer so an IO can be pointed straight at it.
...
//
// It answers BufferedConsoleOutput::fetch, emptying included: a second Fetch
// with nothing written between them returns empty, which is what lets one
// buffer serve a sequence of commands without the second reading the first's
// output.
