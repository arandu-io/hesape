...
// declared.
//
// It is what makes Schedule.Daily().Group(...) work: the frequency is named
// before there is an event to put it on, so it is held here and merged into
// every event the group declares.
//
// It embeds an Event rather than repeating the seventy frequency and
// attribute methods. The event it embeds is never run: it is a bag of
// attributes with the methods already on it.
...
//
// Go has no fallthrough for a missing method, so a schedule-level call in a
// chain that started from Attributes reaches it through this instead.
...
// themselves.
//
// The attributes have no mutex to register a skip against, so it only
...
//
// Only what was actually set is copied, so an event that named its own
// timezone keeps it.
