...
// not say.
//
// It is the deadlock protection and nothing else: a process that dies holding
// the lock releases it when this runs out, so it is sized above the longest
// isolated command rather than tight against the usual one.
...
// and every method returns an error alongside the bool, because that round
// trip can fail and a mutex that reports "not held" when it could not ask is
// a mutex that lets both copies run.
...
//
// It is the one the framework wires: the lock is the same one cache.Locks
// issues, so an isolated command and a scheduled task contend through one
...
// Exists reports true for the lock this process took, without going to the
// store to check.
//
// For a lock this process does not hold, it acquires the lock to find out and
// releases it again immediately.
...
//
// It is keyed on Isolated rather than on the name when the command sets one,
// which is what lets two commands that must not overlap each other share a
// lock -- a nightly reconciliation and the manual command that does the same
// work name the same lock, and neither starts while the other holds it.
//
// The key carries no tenant, and that is deliberate: a lock per tenant would
// let N replicas each run the command for a different tenant at the same
// time, which is the problem and not the solution. See docs/15.
