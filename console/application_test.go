...
// newRegistry is the shape every test here starts from: a application over two
// buffers and a script of answers.
func newRegistry(t *testing.T, input string) (*console.Application, *bytes.Buffer, *bytes.Buffer) {
	t.Helper()
	var out, errOut bytes.Buffer
	return console.NewApplication(&out, &errOut, strings.NewReader(input)), &out, &errOut
}
...
func TestAddRefusesWhatCannotWork(t *testing.T) {
	for name, add := range map[string]func(*console.Application){
		"no name": func(r *console.Application) { r.Add(console.Command{Run: nothing}) },
		"no Run":  func(r *console.Application) { r.Add(console.Command{Name: "migrate"}) },
		"twice": func(r *console.Application) {
