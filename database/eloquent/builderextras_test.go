...
func TestNewEloquentBuilderIsWhatNewModelQueryGoesThrough(t *testing.T) {
	model, _ := newUserModel()
	...
	b := model.NewEloquentBuilder(base)
	if b.GetModel() != nil {
		t.Error("newEloquentBuilder does not set the model: newModelQuery does that after, as there")
	}
	if b.GetQuery() != base {
