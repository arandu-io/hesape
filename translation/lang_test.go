...
func TestTheBundledCatalogueAnswersTheFrameworkGroups(t *testing.T) {
	...
	for key, want := range map[string]string{
		...
		if got := tr.T("en", key, nil); got != want {
...
func TestTheBundledCatalogueInterpolates(t *testing.T) {
	...
	got := tr.T("en", "auth.throttle", translation.Replace{"seconds": 60})
	if want := "Too many login attempts. Please try again in 60 seconds."; got != want {
		t.Errorf("T = %q, want %q", got, want)
	}

	got = tr.T("en", "validation.between.numeric", translation.Replace{"attribute": "age", "min": 18, "max": 120})
...
func TestTheApplicationCatalogueOverridesABundledLine(t *testing.T) {
	...
	if got, want := tr.T("en", "auth.failed", nil), "We do not know that email and password."; got != want {
		t.Errorf("T = %q, want %q", got, want)
	}
	// Everything the application did not override still answers.
	if got, want := tr.T("en", "auth.password", nil), "The provided password is incorrect."; got != want {
		t.Errorf("T = %q, want %q", got, want)
	}
}
...
func TestALocaleCatalogueWinsOverTheBundledEnglish(t *testing.T) {
	...
	if got, want := tr.T("pt-BR", "auth.failed", nil), "Estas credenciais não correspondem aos nossos registros."; got != want {
		t.Errorf("T = %q, want %q", got, want)
	}
	// And a group it has not translated falls through to English rather than
	// showing the key.
	if got, want := tr.T("pt-BR", "passwords.reset", nil), "Your password has been reset."; got != want {
		t.Errorf("T = %q, want %q", got, want)
	}
}
