...
//	SecLibGateway.*                                          -- the phpseclib driver
//
...
// the same five-line loop five times). In Go that loop is
...
//
// SecLibGateway.put and get use phpseclib's SFTP. There is no SFTP client in
// golang.org/x/crypto -- it stops at the SSH transport -- so this is the single
// surface that would need a dependency the CI rejects, or a seventh submodule
// under the ADR 0048 pattern whose only consumer is a component Laravel deleted.
// Writing SCP by hand instead is not a saving: it is a wire format with a known
// class of client-side path-traversal bugs (CVE-2019-6111), which is a poor
// thing to reimplement to avoid a dependency.
//
// ## A named task -- no
//
...
// Illuminate's. Connection.status is result.ExitCode, and Connection.display is
// what an OutputHandler passed in place of that nil does, line by line.
//
// MultiConnection.run and RemoteManager.group -- the same command on a group:
//
...
//
// Connection.put and putString -- send a file:
//
